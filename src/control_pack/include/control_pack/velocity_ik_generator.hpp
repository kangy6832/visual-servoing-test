...
#ifndef VELOCITY_IK_GENERATOR_HPP
#define VELOCITY_IK_GENERATOR_HPP
     ...
     class DimensionMismatchException : public std::runtime_error {
        public:
            ...
            DimensionMismatchException(int expected, int actual, const std::string& context) : 
                std::runtime_error(std::string("Dimension mismatch in ") + context + std::string(": expected ") + 
                std::to_string(expected) + std::string(", got ") + std::to_string(actual)){}
     };
...
#endif // VELOCITY_IK_GENERATOR_HPP
